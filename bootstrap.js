...
// Function to show the payment form when a payment icon is clicked
function showPaymentForm(paymentMethod) {
  const paymentForm = document.getElementById('payment-form');
  paymentForm.style.display = 'block';
  
  // Optionally, you can display the selected payment method
...
function handlePaymentFormSubmit(event) {
  ...
  if (validateCardDetails(cardNumber, cardholderName, expiryDate, cvv)) {
    ...
    document.getElementById('payment-form').style.display = 'none'; // Hide the form after submission
  } else {
...
function validateCardDetails(cardNumber, cardholderName, expiryDate, cvv) {
...
// Cart object to hold cart items and count
const cart = {
  items: [],
  itemCount: 0,
};
...
function addToCart(item) {
  ...
  updateCartCount();
  updateCartDropdown();
  console.log(`Added to cart: ${item.name}. Total items: ${cart.itemCount}`);
}
...
function updateCartDropdown() {
  ...
  totalItemsElement.textContent = cart.itemCount;
}
...
function removeFromCart(itemName) {
  ...
  if (itemIndex > -1) {
      ...
      updateCartCount();
      updateCartDropdown();
      console.log(`Removed from cart: ${itemName}. Total items: ${cart.itemCount}`);
  } else {
